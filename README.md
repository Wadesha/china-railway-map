...
> 子站说明：本地通过**已连接的腾讯地图连接器**采集了真实数据——27 个枢纽站真实坐标（geocoder）、27 个枢纽周边真实 POI（地铁站/餐饮/酒店/旅游景点，共 981 条）、5 组城际真实乘车方案（directionTransit，公交/地铁/高铁混合）——并内联打包进静态页。访问者**无需任何运行时 key** 即可查看真实数据。底图仍用 ECharts 矢量渲染（合规、含完整国界）。连接器代理仅在 WorkBuddy 内有效、无法跟随部署，故采用「本地采集→快照内联」方式；如需实时调用可自行在源码 `TENCENT_KEY` 填 key。
>
> POI 数据源说明：原始 **14 大枢纽**（北京/上海/广州/武汉/成都/西安/郑州/南京/长沙/杭州/天津/济南/贵阳/昆明）由 `placeSearchNearby`（站点 2.5km 半径，含真实距离）采集；**新增 13 枢纽**（石家庄/南昌/连云港/徐州/兰州/西宁/格尔木/拉萨/哈尔滨/长春/沈阳/大连/宝鸡）因 `placeSearchNearby` 代理临时降级，改用 `placeSuggestion`（全市/全区推荐口径，无距离字段）采集，距离由站心坐标 haversine 自算并按就近排序、按类别上限过滤（地铁站/餐饮/酒店 ≤30km、旅游景点 ≤80km）。suggestion 口径偏「全市推荐」，景点/地铁站可能距站较远，属已知精度差异。
...
│   ├── poi.json            # 27 个枢纽周边真实 POI（地铁站 / 餐饮 / 酒店 / 旅游景点，共 981 条）
